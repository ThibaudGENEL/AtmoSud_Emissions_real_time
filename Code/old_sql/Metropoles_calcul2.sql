-- METHODE NUMERO 2
--> on considère que toute variation de conso > [conso "de base" sans Chauff ni clim]  est dûe au chauffage ou à la clim.
-- permet de fixer toutes les consos autres, et on fait varier quotidiennement chauffage et clim en fonction de la température

drop table if exists bases_conso;
create temp table bases_conso as
select id_comm, SUM(val) / 365 as Base_conso
from total.bilan_comm_v11_diffusion natural join commun.tpk_polluants natural join commun.tpk_usages natural join total.tpk_cat_energie_color tcec
where nom_court_polluant = 'conso'
and tcec.nom_court_cat_energie = 'Electricité'
	and nom_usage not in ('Chauffage', 'Climatisation')
	and an = (select max(an) from total.bilan_comm_v11_diffusion)  -- 2021 là
	and format_detaille_scope_2=1
	and id_comm/1000 in (4,5,6,13,83,84) 
	--and id_secteur_detail in (3, 4)
group by id_comm;
	
drop table if exists bases_conso_metropoles;
create temp table bases_conso_metropoles as
select *, LEAST(base_conso, consommation_mwh) as base_conso_mwh, consommation_mwh - LEAST(base_conso, consommation_mwh) as conso_chauff_clim
	from (select CASE 
	        WHEN id_comm / 1000 = 6 THEN 6100
	        WHEN id_comm / 1000 = 83 THEN 83100
	        when id_comm/1000 = 13 then 13000
	        ELSE NULL
	    END AS id_metropole, 
	    SUM(base_conso) as base_conso
	from bases_conso
	where id_comm in (06006, 06009, 06011, 06013, 06020, 06021, 06025, 06027, 06032, 06033, 06034, 06039, 06042, 06046, 06054, 06055, 06059, 06060, 06064, 06065, 06066, 06072, 06073, 06074, 06075, 06080, 06088, 06102, 06103, 06109, 06110, 06111, 06114, 06117, 06119, 06120, 06121, 06122, 06123, 06126, 06127, 06129, 06144, 06146, 06147, 06149, 06151, 06153, 06156, 06157, 06159,
	/* Aix-Marseille */ 13001, 13002, 13003, 13005, 13007, 13008, 13009, 13012, 13013, 13014, 13015, 13016, 13019, 13020, 13119, 13021, 13022, 13023, 13024, 13025, 13026, 13028, 13029, 13118, 13030, 13031, 13032, 13033, 13035, 13037, 13039, 13040, 13041, 13042, 13043, 13044, 13046, 13047, 13048, 13049, 13050, 13051, 13053, 13054, 13055, 13056, 13059, 13060, 13062, 13063, 13069, 13070, 13071, 84089, 13072, 13073, 13074, 13075, 13077, 13078, 13080, 13079, 13081, 13082, 13084, 13085, 13086, 13087, 13088, 13090, 13091, 13092, 13093, 13095, 13098, 13099, 13101, 13102, 83120, 13103, 13104, 13105, 13106, 13107, 13109, 13110, 13111, 13112, 13113, 13114, 13115, 13117,
	/* Toulon metropole */ 83034, 83047, 83062, 83069, 83090, 83098, 83103, 83126, 83129, 83137, 83144, 83153)
	group by id_metropole) as bases
natural join prj_res_tps_reel.src_conso_elec_jour_test_hist scejt;
select * from bases_conso_metropoles;



-- CORRECTION AVEC TEMPERATURE

-- connexion à prisme pour import meteo
/*
create extension if not exists postgres_fdw;
CREATE SERVER fdw_prisme
FOREIGN DATA WRAPPER postgres_fdw
OPTIONS (host '172.16.13.168', port '5432', dbname 'prisme', updatable 'false');
create user mapping for user
	server fdw_prisme
	options ( user 'emi', password 'Em1ss!');   -- le mapping est crée */
drop foreign table if exists prj_res_tps_reel.src_meteo_hist;
create foreign table prj_res_tps_reel.src_meteo_hist(
	id_station INT,
	date_tu timestamp,
	temperature FLOAT
	)
server fdw_prisme
options (schema_name 'meteo', table_name 'data_station_meteo');

-- group by JOUR et Calcul DJ17, DJ21
drop table if exists prj_res_tps_reel.w_meteo_hist;
create table prj_res_tps_reel.w_meteo_hist as
select t.id_station, CAST(t.date_tu AS DATE) AS date,
    AVG(t.temperature) AS temperature,
    case
		when AVG(t.temperature) < 17 then 17 - AVG(t.temperature)
		else 0
	end as DJ17,
	case 
		when AVG(t.temperature) > 21 then AVG(t.temperature) - 21
		else 0
	end as DJ21
from prj_res_tps_reel.src_meteo_hist t
GROUP by t.id_station, CAST(t.date_tu AS DATE)
ORDER by date desc, t.id_station;
;	
comment on table prj_res_tps_reel.w_meteo_hist is 'Température, DJ17 et DJ21 quotidiens pour chaque commune en Région Sud jusqu à aujourd hui';
select * from prj_res_tps_reel.w_meteo_hist;
-- group by metropole
drop table if exists meteo_metropole;
create temp table meteo_metropole as
select Date, 
	CASE 
    	WHEN id_station/1000 = 6 THEN 6100
    	WHEN id_station/1000 = 83 THEN 83100
    	when id_station/1000 = 13 then 13000
		ELSE NULL
    END AS id_metropole, AVG(temperature) as Temperature,
    case
		when AVG(Temperature) < 17 then 17 - AVG(Temperature)
		else 0
	end as DJ17,
	case
		when AVG(Temperature) > 21 then AVG(Temperature) - 21
		else 0
	end as DJ21
from prj_res_tps_reel.w_meteo_hist
where id_station in (06006, 06009, 06011, 06013, 06020, 06021, 06025, 06027, 06032, 06033, 06034, 06039, 06042, 06046, 06054, 06055, 06059, 06060, 06064, 06065, 06066, 06072, 06073, 06074, 06075, 06080, 06088, 06102, 06103, 06109, 06110, 06111, 06114, 06117, 06119, 06120, 06121, 06122, 06123, 06126, 06127, 06129, 06144, 06146, 06147, 06149, 06151, 06153, 06156, 06157, 06159,
/* Aix-Marseille */ 13001, 13002, 13003, 13005, 13007, 13008, 13009, 13012, 13013, 13014, 13015, 13016, 13019, 13020, 13119, 13021, 13022, 13023, 13024, 13025, 13026, 13028, 13029, 13118, 13030, 13031, 13032, 13033, 13035, 13037, 13039, 13040, 13041, 13042, 13043, 13044, 13046, 13047, 13048, 13049, 13050, 13051, 13053, 13054, 13055, 13056, 13059, 13060, 13062, 13063, 13069, 13070, 13071, 84089, 13072, 13073, 13074, 13075, 13077, 13078, 13080, 13079, 13081, 13082, 13084, 13085, 13086, 13087, 13088, 13090, 13091, 13092, 13093, 13095, 13098, 13099, 13101, 13102, 83120, 13103, 13104, 13105, 13106, 13107, 13109, 13110, 13111, 13112, 13113, 13114, 13115, 13117,
/* Toulon metropole */ 83034, 83047, 83062, 83069, 83090, 83098, 83103, 83126, 83129, 83137, 83144, 83153) 
	and extract(year from date) >= 2021
group by id_metropole, date
order by date desc, id_metropole
;
select * from meteo_metropole;
-- DJ de base par metropole :
select * from prj_res_tps_reel.w_bases_dj_metropoles;




-- Consommations par usage, energie
drop table if exists consos_u_e;
create temp table consos_u_e as
select id_metropole, date, temperature, base_conso_mwh, consommation_mwh, id_usage, usage_old, code_cat_energie, energie_old, ratio_conso, 
	case 
		 when id_usage not in (0, 21) then base_conso_mwh * ratio_conso
		 when id_usage = 0 then consommation_mwh * ratio_conso * (dj17/base_dj17) * (365 / 153)   -- 153 jours de chauff, à retrouver
		 when id_usage = 21 then consommation_mwh * ratio_conso * (dj21/base_dj21) * (365 / 51)   -- 51 jours de clim, à retrouver
	end as conso_u_e
from bases_conso_metropoles natural join prj_res_tps_reel.w_ratios_conso_metropoles wrcm 
	natural join meteo_metropole natural join prj_res_tps_reel.w_bases_dj_metropoles wbdm 
order by date desc, id_metropole, id_usage, code_cat_energie;
select * from consos_u_e;


-- validation consos usage energie. Comparaison 2021
select * from prj_res_tps_reel.w_consos_u_e_test_hist;
drop table if exists comparaison2021;
create temp table comparaison2021 as
select id_metropole, id_usage, nom_usage, code_cat_energie, nom_court_cat_energie, conso_tps_reel_ann, conso_inventaire_ann,  
	case 
		when conso_inventaire_ann = 0 then null
		else conso_tps_reel_ann / conso_inventaire_ann
	end as Rapport 
from
	(select id_metropole, id_usage, code_cat_energie, sum(conso_u_e) as conso_tps_reel_ann
	from consos_u_e
	where EXTRACT(year from date) = 2021
	group by id_metropole, id_usage, code_cat_energie) as tps_reel
natural join
	(select CASE 
	    	WHEN id_comm/1000 = 6 THEN 6100
	    	WHEN id_comm/1000 = 83 THEN 83100
	    	when id_comm/1000 = 13 then 13000
			ELSE NULL
	    END AS id_metropole,
	    id_usage, code_cat_energie, sum(val) as conso_inventaire_ann
	from total.bilan_comm_v11_diffusion bcvd natural join commun.tpk_polluants tp
	where an = 2021
		and tp.nom_court_polluant = 'conso'
		and format_detaille_scope_2=1
		--and id_secteur_detail in (3, 4)
		and id_comm in (06006, 06009, 06011, 06013, 06020, 06021, 06025, 06027, 06032, 06033, 06034, 06039, 06042, 06046, 06054, 06055, 06059, 06060, 06064, 06065, 06066, 06072, 06073, 06074, 06075, 06080, 06088, 06102, 06103, 06109, 06110, 06111, 06114, 06117, 06119, 06120, 06121, 06122, 06123, 06126, 06127, 06129, 06144, 06146, 06147, 06149, 06151, 06153, 06156, 06157, 06159,
/* Aix-Marseille */ 13001, 13002, 13003, 13005, 13007, 13008, 13009, 13012, 13013, 13014, 13015, 13016, 13019, 13020, 13119, 13021, 13022, 13023, 13024, 13025, 13026, 13028, 13029, 13118, 13030, 13031, 13032, 13033, 13035, 13037, 13039, 13040, 13041, 13042, 13043, 13044, 13046, 13047, 13048, 13049, 13050, 13051, 13053, 13054, 13055, 13056, 13059, 13060, 13062, 13063, 13069, 13070, 13071, 84089, 13072, 13073, 13074, 13075, 13077, 13078, 13080, 13079, 13081, 13082, 13084, 13085, 13086, 13087, 13088, 13090, 13091, 13092, 13093, 13095, 13098, 13099, 13101, 13102, 83120, 13103, 13104, 13105, 13106, 13107, 13109, 13110, 13111, 13112, 13113, 13114, 13115, 13117,
/* Toulon metropole */ 83034, 83047, 83062, 83069, 83090, 83098, 83103, 83126, 83129, 83137, 83144, 83153) 
	group by id_metropole, id_usage, code_cat_energie) as inv
 natural join total.tpk_cat_energie_color tcec natural join commun.tpk_usages tu 
	;
select * from comparaison2021;

